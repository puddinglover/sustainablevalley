<?php require('header.php'); ?>

<?php require('nav.php'); ?>

<section class="ticket wrapper">
    <div class="page-name">
        <h1>Tickets</h1>
        <p>This is a short description of what the page contains and what the user can read/See here!</p>
    </div>
</section>
<div class="bgPicture"> </div>
 <section class="tickets" id="main-ticket">

        <div class="wrapper">
            <div class="tickets__card">
                <h2>Partoutticket</h2>
                <img class="ticket__picture"src="images/partoutticket.jpg" alt="partout ticket for the whole festival">
                <article class="">
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                </article>
                <div class="button button--cards">
                    <a href="#">Buy</a>
                </div>
            </div>
            <div class="tickets__card">
                <h2>One day ticket</h2>
                <img class="ticket__picture"src="images/1dayticket.jpg" alt="one day only ticket">
                <article class="">
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                </article>
                <div class="button button--cards">
                    <a href="#">Buy</a>
                </div>
            </div>
            <div class="tickets__card">
                <h2>Support ticket</h2>
                <img  class="ticket__picture"src="images/supportticket.jpg"  alt="support ticket">
                <article class="">
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
                </article>
                <div class="button button--cards">
                    <a href="#">Buy</a>
                </div>
            </div>
        </div>
    </section>
<?php require('footer.php'); ?>
